 ...
 * Module 3 — Appointment Management  : getDoctorAppointments(), updateAppointmentStatus()
 * Module 4 — Diagnosis & Treatment   : createMedicalRecord(), updateMedicalRecord()
 * Module 5 — Prescription Management : createPrescription(), getPatientPrescriptions()
 * Module 6 — Test Results            : getTestResults(), addTestResultNote()
 *
 * Design patterns:
 *   Builder       — MedicalRecordBuilder, PrescriptionBuilder
 *   Factory Method — AppointmentStatusHandlerFactory
 *   Bridge        — AppointmentStatusNotification + MessageSender
 */
@Service
public class DoctorService {
    ...
    /**
     * Bridge: email sender injected here.
     * Change @Qualifier("smsSender") to switch delivery channel with no other code change.
     */
    ...
    public DoctorDashboardResponse getDashboard(Integer doctorId) {
        ...
        List<Appointment> todayList =
                appointmentRepository.findByDoctor_DoctorIdAndAppointmentDate(doctorId, LocalDate.now());
        ...
        return DoctorDashboardResponse.builder()
                ...
                .availabilityStatus(doctor.getAvailabilityStatus() != null
                        ? doctor.getAvailabilityStatus().getDbValue() : null)
    ...
    public PatientProfileResponse getPatientProfile(Integer patientId) {
        ...
        return PatientProfileResponse.builder()
                ...
                .dateOfBirth(patient.getUser().getDateOfBirth() != null
                        ? patient.getUser().getDateOfBirth().toString() : null)
                .gender(patient.getUser().getGender() != null
                        ? patient.getUser().getGender().name() : null)
    ...
    public List<AppointmentResponse> getDoctorAppointments(Integer doctorId, AppointmentStatus status) {
        ...
        List<Appointment> appointments = (status != null)
                ...
                : appointmentRepository.findByDoctor_DoctorIdOrderByAppointmentDateAscAppointmentTimeAsc(doctorId);
    ...
    /**
     * Updates an existing MedicalRecord — e.g., doctor adds notes after lab results arrive.
    ...
    public PrescriptionResponse createPrescription(Integer doctorId, PrescriptionRequest request) {
        ...
        for (PrescriptionRequest.PrescriptionDetailRequest detail : request.getDetails()) {
            ...
            builder.addMedication(medication, detail.getDosage(), detail.getFrequency(), detail.getDuration());
        }
