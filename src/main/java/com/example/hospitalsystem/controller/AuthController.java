...
import com.example.hospitalsystem.model.Admin;
    ...
    public ResponseEntity<Map<String, Object>> register(@RequestBody RegisterRequest request) {
        ...
        try {
            ...
            if (request.getUsername() != null) {
                ...
                newPatient.setLastName(names.length > 1 ? names[1] : "");
            } else {
                newPatient.setFirstName("Unknown");
    ...
    public ResponseEntity<Map<String, Object>> login(@RequestBody AuthRequest request) {
        ...
        try {
            ...
            if (patientOpt.isPresent()) {
                return processLogin(patientOpt.get().getPassword(), password, email, "Patient", patientOpt.get());
            }

            // 2. Check in Doctor Repository
            Optional<Doctor> doctorOpt = doctorRepository.findByEmail(email);
            if (doctorOpt.isPresent()) {
                return processLogin(doctorOpt.get().getPassword(), password, email, "Doctor", doctorOpt.get());
            }
    ...
    private ResponseEntity<Map<String, Object>> processLogin(String storedHash, String rawPassword, String email,
        ...
        if (BCrypt.checkpw(rawPassword, storedHash)) {
            ...
            if (role.equals("Patient")) {
                ...
                userData.put("firstName", p.getFirstName());
                userData.put("lastName", p.getLastName());
            } else if (role.equals("Doctor")) {
                Doctor d = (Doctor) userObj;
                userData.put("id", d.getDoctorId());
                userData.put("firstName", d.getFirstName());
                userData.put("lastName", d.getLastName());
